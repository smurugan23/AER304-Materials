...
def StrainProcess(data: pd.DataFrame, test_num: np.int8, sample_dimensions: np.array):
    ...
    # Rename columns
    parsed_data = data.rename(columns={'X_Value': 't', 'Untitled': 'MTS_F', 'Untitled 1': 'MTS', 'Untitled 2': 'Laser', 'Untitled 3': 'Strain Gauge 1', 'Untitled 4': 'Strain Gauge 2', 'Untitled 5': 'MTS_stress'})
    ...
    parsed_data['Strain Gauge 1'] = -1 * parsed_data['Strain Gauge 1'][0:730] # filtering out some 'post gauge break' 
    parsed_data['Strain Gauge 2'] =      parsed_data['Strain Gauge 2'][0:730]
...
def ModulusProcess(data: pd.DataFrame, test_num: np.int8):
    ...
    if test_num == 3:
        ...
        modulus[2], intercept = np.polyfit(data['Strain Gauge 1'][0:end_ind], data.MTS_stress[0:end_ind], deg=1) # transverse
    else:
        modulus[0], intercept = np.polyfit(data['Laser'][30:end_ind], data.MTS_stress[30:end_ind], deg=1)
        modulus[1], intercept = np.polyfit(data['Strain Gauge 2'][0:end_ind], data.MTS_stress[0:end_ind], deg=1)
        modulus[2], intercept = np.polyfit(data['Strain Gauge 1'][0:end_ind], data.MTS_stress[0:end_ind], deg=1) # transverse
...
def YieldProcess(data: pd.DataFrame, test_num: np.int8, modulus: np.array):
    ...
    if test_num != 3:
        # plt.plot(strain_gauge, stress_measured)
        # plt.plot(strain_gauge, stress_offset_gauge)

        diff = abs(stress_measured - stress_offset_gauge)
        intersect_ind = list(diff).index(min(diff))
        yield_strength_gauge = [strain_gauge[intersect_ind], stress_measured[intersect_ind]]
    
    yield_strength = [yield_strength_laser, yield_strength_gauge]
